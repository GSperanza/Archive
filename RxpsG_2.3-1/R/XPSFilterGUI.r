...
XPSFilter <- function() {
   ...
   resize <- function(data){
      FiltAmpli <<- FALSE         #preserve ampliture range of original data
      data <- BkgSubtraction(data)
      data <- normalize(data)
      Min <- AmpliRng[1]
      Max <- AmpliRng[2]
...
#--- Autoregressive filter
         ...
         Filtrato <<- stats::filter(x=DaFiltrare, filter=rep(1,2*Ro),  sides=2, method="convolution", circular=FALSE)  #voglio usare FILTER pacchetto STATS non pacchetto SIGNAL
...
#here set level in the range [1:Nlevel] and threshold in [0.1:1]
   ...
   ButtFilt <- function() {
      ...
      CutOFF <- (20.001-CutOFF)/20    #here cut-off were freq/Nyquist for example 40/500Hz (sampling freq = 1kHz) where 40Hz is adapted to the typical XPS noise
      coefficenti <- NULL
      DaFiltrare <<- NULL
      Filtrato <<- NULL
      #preprocessing
...
# --- Tab5 ---
   ...
   F6FiltOrder <- gcombobox(c(10,20,40,60), selected=1, editable=FALSE, handler=function(h,...){
                     enabled(F6CutOFF) <- TRUE
                 }, container = F6frame1)
   F6frame2 <- gframe(text="Degree of Noise Rejection", spacing=5, container=F6group2)
   F6CutOFF <- gcombobox(c(1:20), selected=-1, editable=FALSE, handler=function(h,...){
...
# --- Tab6 ---
   ...
   F7CutOFF <- gcombobox(c(1:20), selected=-1, editable=FALSE, handler=function(h,...){
