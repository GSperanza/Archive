...
#--- Y normalization == scale c(0,1)
    ...
    if (PlotParameters$Normalize) {
			    ...
			    if (PlotParameters$NormPeak > 0) {
			        PeakIdx <- findXIndex(unlist(X[[1]]),  PlotParameters$NormPeak)
cat("\n 2222", PeakIdx)
          LL<-length(Y)
          for (idx in 1:LL)) {
              yy <- lapply(Y[[idx]], function(y) {y[PeakIdx]} )
          }
print(yy)
			        maxy <- sapply(yy, function(x) max(sapply(x, max))) #here yy is the a region around the selected peak selected for normalization
print(maxy)
       }
